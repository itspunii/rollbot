...
var safeBetAmount = 1, greenBetAmount = 1, doubleGreenBetAmount = 10;
var currentBalance, timerInt, gemsOnStart, currentJackpot;
...
function start() {
    ...
    //$("#balanceAmountMain").eq(2).attr("id", "jackpot");
    $(".test:nth-child(2)").attr("id", "jackpot")
    //currentBalance = Number(document.getElementById("balanceAmountMain").innerHTML.replace(/<|!|-|>/g, ""));
    currentBalance = Number($("#balanceAmountMain:eq(0)").text());
    currentJackpot = Number($("#jackpot:nth-child(2)").text());
    ...
    //checkAllIds(); Find a way to check ids.
    timerInt = setInterval(getCurrentTimer, 1000);

    $("#startRollButton").css("backgroundColor", "red");
    $("#startRollButton").css("color", "white");
    $("#startRollButton").attr("onclick", "stop()");
    $("#startRollButton").text("Stop");
}
...
function stop() {
    ...
    timerInt = null;
    console.log("Stopped.");
    $("#menuOutput").text(`Stopped betting.`);

    $("#startRollButton").css("backgroundColor", "lime");
    $("#startRollButton").css("color", "black");
    $("#startRollButton").attr("onclick", "start()");
    $("#startRollButton").text("Start");
}

function createMenu() {
    //Main roll menu
    $("div.left").append("<button id='rollMenuButton' onclick='rollMenu()' style='z-index:9999; background-color: #20202300; width: 100px; border: solid lime 1px'>roll</button>");
    ...
    $("#rollMenuDiv").append("<button id='showStatsMenu' onclick='statsMenu()' style='position: absolute; left: 75%; top: 35px; z-index: 998'>Stats</button>");
    $("#rollMenuDiv").append("<button id='showOverridesMenu' onclick='overridesMenu()' style='position: absolute; left: 75%; top: 65px; z-index: 998'>Overrides</button>");
    ...
    $("#rollMenuDiv").append("<input id='GreenBetAmountInput' style='position: absolute; left: 10px; top: 35px; width: 150px;' placeholder='Green bet amount'>");
    $("#rollMenuDiv").append("<input id='doubleGreenBetAmountInput' style='position: absolute; left: 10px; top: 65px; width: 150px;' placeholder='3x green bet amount'>");
    ...
    $("#rollMenuDiv").append("<input id='trainAmountInput' style='position: absolute; left: 250px; top: 65px; width: 150px;' placeholder='Train amount'>");

    //Start/Stop button
    $("#rollMenuDiv").append("<button id='startRollButton' style='position: absolute; right: 15%; bottom: 10px; background-color: lime; color: black; width: 100px' onclick='start()'>Start</button>");
...
function setTimerid() {
    $("cw-next-roll:first").attr("id", "timerMain");
    document.getElementById("timerMain").firstChild.setAttribute("id", "timerParent");
    document.getElementById("timerParent").children[1].setAttribute("id", "timer");
    $(".rolls.d-flex").attr("id", "lastRoll");
}
...
function setSideButtonsid() {
    $("[data-test=x2]").attr("id", "x2Button");
    $("[data-test=1-div-2]").attr("id", "div2Button");
    $("[data-test=plus-1]").attr("id", "plus1Button");
    $("[data-test=clear]").attr("id", "clearButton");
}
...
function setBetAmount(x) {
    $("#clearButton").click();
    for (let y = 0; y < x; y++) {
        $("#plus1Button").click();
    }
    if (x != safeBetAmount) {
        console.log(`Set bet amount to ${x}`);
    }
...
function checkAllIds() {
    ...
    if (!$("#balanceAmountMain").attr("id") == "balanceAmountMain") {
        console.log("balanceAmountMain ID not set. Refresh and try again.");
    }
    if (!$("#lastRoll").attr("id") == "lastRoll") {
        console.log("lastRoll ID not set. Refresh and try again.");
    }
    if (!$("greenBetButton").attr("id") == "greenBetButton") {
        console.log("greenBetButton ID not set. Refresh and try again.");
    }
    if (!$("redBetButton").attr("id") == "redBetButton") {
        console.log("redBetButton ID not set. Refresh and try again.");
    }
    if (!$("blackBetButtonn").attr("id") == "blackBetButton") {
        console.log("blackBetButton ID not set. Refresh and try again.");
    }
    if (!$("div2Button").attr("id") == "div2Button") {
        console.log("div2Button ID not set. Refresh and try again.");
    }
    if (!$("plus1Button").attr("id") == "plus1Button") {
        console.log("plus1Button ID not set. Refresh and try again.");
    }
    if (!$("clearButton").attr("id") == "clearButton") {
        console.log("clearButton ID not set. Refresh and try again.");
    }
}
...
function getLastRoll() {
    ...
    if (document.getElementById("lastRoll").firstChild.className.includes("bg-red")) { //Red
        ...
        console.log("Last roll is green");
        console.log(`Green is on a ${greenStreak} streak.`);
        console.log(" ");
    }
...
function applySettings() {
    ...
    if ($("#GreenBetAmountInput").val() != "") {
        if ($("#GreenBetAmountInput").val() != 0) {
       GreenBetAmount = $("#GreenBetAmountInput").val();
       console.log(`Green bet amount set to ${GreenBetAmount}`);
       $("#GreenBetAmountInput").val("")
   } else {
       console.log(`Invalid value in GreenBetAmount.`);
       }
   }  
...
/*Changelog
    ...
    Added Overrides in Overrides tab:
        Overrides default bet amount when balance is higher or equals to a set value.
        Ex.: Set Balance to 200 and Bet amount to 5: When balance is higher or equal to 200, bet 5.
